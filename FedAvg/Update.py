...
class LocalUpdate(object):
    ...
    def update_weights_fedprox(self, net):
        global_net = dict()
        ...
        if self.args.guass == True:
            privacy_engine = PrivacyEngine(
                net,
                batch_size=self.args.local_bs,
                sample_size=self.dataset_size,
                alphas=[1 + x / 10.0 for x in range(1, 100)] + list(range(12, 64)),
                noise_multiplier=self.args.noise_multiplier,#noise_multiplier = (math.sqrt(2.0 * math.log(1.25 / self.args.delta)) / (self.args.epsilon * self.args.epochs * self.args.local_ep)),#
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                #     proximal_term = 0.0
                #     upnet = net.state_dict()
                #     # iterate through the current and global model parameters
                #     for w, w_t in zip(upnet.parameters(), global_net.parameters()):
                #         # update the proximal term
                #         # proximal_term += torch.sum(torch.abs((w-w_t)**2))
                #         #proximal_term += (w - w_t).norm(2)
                #         proximal_term += ((self.args.mu / 2) * torch.norm((w - w_t))**2)
                #         # div1 = torch.mean(
                #         #     torch.abs(torch.addcdiv(torch.zeros_like(w), 1, w, w_t, out=None)))
                #         # div2 = torch.mean(
                #         #     torch.abs(torch.addcdiv(torch.zeros_like(w), 1, w_t, w, out=None)))
                #     # print("proxmal_term为{}".format(proximal_term))
                #     loss = self.loss_func(log_probs, labels) + (self.args.mu / 2) * proximal_term  # (self.mu / 2)
                # else:
                norm = self.get_l2_norm(net.parameters(), origin_net.parameters())
                loss = self.loss_func(log_probs, labels) + self.args.mu / 2 * norm
                #loss = self.loss_func(log_probs, labels)
                loss.backward()
                if self.args.guass == True:
                    virtual_batch_rate = int(self.args.virtual_batch_size / self.args.local_bs)
                    # take a real optimizer step after N_VIRTUAL_STEP steps t
                    if ((batch_idx + 1) % virtual_batch_rate == 0) or ((batch_idx + 1) == len(self.ldr_train)):
                        optimizer.step()
                        optimizer.zero_grad()
                    else:
